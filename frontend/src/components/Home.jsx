...
export default function Home({ onNavigate, isDarkMode, toggleTheme, user, onLogout }) {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'dark bg-[#0a0f1c] text-white' : 'bg-slate-200 text-slate-900'} flex flex-col font-sans transition-colors duration-500 selection:bg-indigo-500/30`}>
      
      {/* Navbar / Header */}
      <nav className="w-full bg-slate-200/90 dark:bg-[#0a0f1c]/80 backdrop-blur-xl border-b border-slate-300/50 dark:border-slate-800/50 sticky top-0 z-50 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 font-bold text-sm text-slate-600 dark:text-slate-400">
          ...
          <div className="flex items-center gap-2 sm:gap-4">
             ...
             <button 
                onClick={toggleTheme}
                className="p-2.5 text-slate-600 dark:text-slate-400 hover:bg-slate-300/50 dark:hover:bg-slate-800/50 rounded-full transition-all duration-300 focus:outline-none hover:scale-110 active:scale-95"
             ...
             {user ? (
               <>
                 <div className="hidden sm:flex items-center gap-2 font-bold text-slate-700 dark:text-slate-200 px-4 py-2 bg-slate-300/50 dark:bg-slate-800/50 rounded-full border border-slate-300 dark:border-slate-700/50">
                   <User size={16} className="text-indigo-600 dark:text-indigo-400" />
                   <span>{user.name || 'User'}</span>
                 </div>
                 <button 
                    onClick={onLogout}
                    className="flex items-center gap-2 bg-red-100 text-red-600 hover:bg-red-200 dark:bg-red-500/10 dark:hover:bg-red-500/20 dark:text-red-400 font-bold py-2 sm:py-2.5 px-4 sm:px-6 rounded-full transition-all duration-300 active:scale-95 text-sm sm:text-base border border-red-300 dark:border-red-500/20 hover:shadow-lg hover:shadow-red-500/10"
                 >
               ...
               <>
                 <button 
                    ...
                    className="hidden sm:block font-bold text-slate-700 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors px-4 py-2"
                 >
                   Log in
                 </button>
                 <button 
      ...
      <main className="flex-1 flex flex-col items-center justify-center text-center px-6 py-24 md:py-32 relative overflow-hidden">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-300/50 dark:bg-white/5 backdrop-blur-sm border border-slate-400/30 dark:border-white/10 text-slate-800 dark:text-slate-300 font-bold text-xs uppercase tracking-widest mb-8 animate-in fade-in slide-in-from-bottom-4 duration-1000 shadow-sm">
          <Sparkles size={14} className="text-indigo-600 dark:text-indigo-500 animate-pulse" /> 
        ...
        <p className="text-lg md:text-xl lg:text-2xl text-slate-700 dark:text-slate-400 max-w-3xl font-medium mb-12 animate-in fade-in slide-in-from-bottom-8 duration-1000 delay-300 leading-relaxed">
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 animate-in fade-in slide-in-from-bottom-10 duration-1000 delay-500 w-full sm:w-auto">
           ...
           {!user && (
             <button 
               onClick={() => onNavigate('login')}
               className="w-full sm:w-auto bg-slate-300/50 dark:bg-slate-900/50 backdrop-blur-sm text-slate-900 dark:text-white border-2 border-slate-400/50 dark:border-slate-800 hover:border-indigo-600/50 dark:hover:border-indigo-400/50 hover:bg-slate-300 dark:hover:bg-slate-900 font-black text-lg py-4 px-10 rounded-full transition-all duration-300"
             >
      ...
      {/* Features Grid */}
      <section className="py-32 bg-slate-200 dark:bg-[#0a0f1c] border-t border-slate-300/50 dark:border-slate-800/50 relative z-10 transition-colors duration-500">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-20">
            ...
            <p className="text-lg md:text-xl text-slate-700 dark:text-slate-400 font-medium max-w-2xl mx-auto">Powered by advanced algorithms that learn and adapt to your body's unique signals.</p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
            {features.map((feature, idx) => (
              <div 
                ...
                className={`p-8 rounded-[2.5rem] bg-slate-100 dark:bg-slate-900/40 border border-slate-300/60 dark:border-slate-800/60 hover:-translate-y-2 transition-all duration-300 cursor-pointer group shadow-xl shadow-slate-300/30 dark:shadow-none hover:shadow-2xl ${feature.hover}`}
              >
                ...
                <p className="text-slate-700 dark:text-slate-400 text-sm md:text-base font-medium leading-relaxed mb-8">
                  {feature.description}
                </p>
                <div className="flex items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-500 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-all">
      ...
      {/* Footer */}
      <footer className="bg-slate-200 dark:bg-[#050811] py-16 border-t border-slate-300/50 dark:border-slate-900 transition-colors duration-500">
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-center gap-8 mb-12">
            ...
            <div className="flex flex-wrap justify-center items-center gap-8 text-slate-600 dark:text-slate-400 font-bold">
          ...
          <div className="w-full h-px bg-slate-300 dark:bg-slate-800/50 mb-8"></div>

          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-slate-500 dark:text-slate-500 text-sm font-medium">
              &copy; {new Date().getFullYear()} FitAI. All rights reserved.
            </p>
            <p className="text-slate-600 dark:text-slate-400 text-sm font-medium flex items-center gap-1.5 bg-slate-300/50 dark:bg-slate-900/50 px-4 py-2 rounded-full border border-slate-300 dark:border-slate-800/50">
